...
of action, followed by a colon. An action strings without colons is
interpreted as a literal string to pass to the tty (as if they were
...
extension of the same name. The extension will be loaded automatically if
neccessary.

Not all extensions define key actions, but popular extensions that do
...
This is a deprecated way of passing key mappings to perl extensions. It is
still supported, but should not be used anymore.

=back

In addition, you can define a range of keysyms in one shot by loading
the C<keysym-list> perl extension and providing a I<string> with pattern
B<list/PREFIX/MIDDLE/SUFFIX>, where the delimiter `/' should be a
