...
games. Zero or negative numbers get negated and directly set the minimum
interval between refreshs, that is, C<10> and C<-0.1> both specify the
same refresh interval (likewise C<50> and C<0.02>). Fractional values are
