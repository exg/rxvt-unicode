...
Before sending me mail, you could go to IRC: C<irc.libera.chat>,
...
doesn't work for you, find a working composite manager or window manager,
both are required to support ARGB visuals for client windows.
...
=begin table
	...
	B<< C<Ps = 2> >>	Clear All
	B<< C<Ps = 3> >>	Like Ps = 0, but is ignored when wrapped (@@RXVT_NAME@@ extension)
...
=begin table
	...
	B<< C<Ps = 776> >>	(urxvt 9.29) Returns info about the character cell size, replies with C<ESC ] 776 ; cell-width ; cell-height ; font-ascent ST>
...
Bits 0, 1, 6 and 7 of B<< C<< <b> >> >> encode the button:

=over

=item Button = B<< C<< (<b> - SPACE) & 0xc3 >> >>
...
=begin table
	...
	128	Button8 pressed
	129	Button9 pressed
	130	Button10 pressed
	131	Button11 pressed

=end table

=back

Bits 2, 3 and 4 of B<< C<< <b> >> >> encode the modifiers when the
button was pressed (X11 mouse report only):

=over

=item Shift = B<< C<< (<b> - SPACE) & 4 >> >>

=item Meta = B<< C<< (<b> - SPACE) & 8 >> >>

=item Control = B<< C<< (<b> - SPACE) & 16 >> >>

=back

Bit 5 of B<< C<< <b> >> >> is set for motion events or double clicks
(rxvt extension, disabled by default):

=over

=item Motion = B<< C<< (<b> - SPACE) & 32 >> >>

=back

C<x> and C<y> encode the coordinates (1|1 is the upper left corner,
just as with cursor positioning):

=over

=item Col = B<< C<< <x> - SPACE >> >>

=item Row = B<< C<< <y> - SPACE >> >>
...
modes, which should be enabled I<before> enabling mouse reporting, for
...
C<b> encodes the button number also on button release (instead of the
value C<3>). The final character of the sequence (M or m) specifies
the event type (press/motion or release).
...
where the parameters are provided as decimal numbers instead of
octets and only C<b> includes an offset of 32.
...
(the basic multilingual page). This increases storage requirements per
character from 2 to 4 bytes. X11 fonts do not yet support these extra
characters, but Xft does.

Please note that rxvt-unicode can store and display unicode characters
above 65535 even without this flag, but the number of such characters is
limited to a few thousand (shared with combining characters, see next
switch).
...
Sets the default C<TERM> value that @@RXVT_NAME@@ sets. The default is
either C<rxvt-unicode> or C<rxvt-unicode-256color>, as appropriate.

=item --with-terminfo=PATH

If set, @@RXVT_NAME@@ will set the environment variable C<TERMINFO> to
the given PATH, which can be useful as a last resort if installing the
