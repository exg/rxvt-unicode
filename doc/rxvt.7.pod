...
Bits 0, 1, 6 and 7 of B<< C<< <b> >> >> indicate the button:

=over

=item Button = B<< C<< (<b> - SPACE) & c3 >> >>
...
Bits 2, 3, 4 and 5 of B<< C<< <b> >> >> indicate the modifiers when the
button was pressed and are added together (X11 mouse report only):

=over

=item State = B<< C<< (<b> - SPACE) & 3c >> >>
...
The lower 2 bits of C<b> encode the button number also on button
