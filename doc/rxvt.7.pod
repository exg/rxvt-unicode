...
=item B<< C<Pm = 1015> >> (B<rxvt-unicode>)
...
The three parameters are C<code>, C<x> and C<y>. Code is the numeric code
as for the other modes (but encoded as a decimal number), C<x> and C<y>
are the coordinates (1|1 is the upper left corner, just as with cursor
positioning).
...
Note that tint, blur and blend effects depend on libAfterImage or on
